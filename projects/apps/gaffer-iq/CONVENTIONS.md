...
- A component that borrows a band token for something that is **not** a rating (an H/A letter, a "postponed" tag, a chip-window bar, a rank-tier chip) must reference the **primitive**, or it silently changes meaning the day the scale is re-pointed under it. This is not hypothetical: `brutal` has been re-pointed twice — the seven-tier scale moved it from red to orange, and it has since moved back to red so the bottom tier could take black — and every borrowed "error" colour in the app would have gone with it, both times.
