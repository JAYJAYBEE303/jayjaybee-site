...
Two other things moved with it. The neutral band widened by two points either side (41–59 → 44–56), because a value that close to even is not worth colouring. And `'good'` moved off green onto blue: the old scale spent two of its five tiers on greens that had to be told apart at 14px on the Full Season ribbon, which they could not be. One green now means one thing.
...
- Anything that borrowed a band colour to mean something that is not a rating — an away fixture, a postponed tag, an import error — reads a **primitive** token instead (`CONVENTIONS.md` §5.3). Left on the semantic token, "error" would have silently turned orange with `brutal`.
