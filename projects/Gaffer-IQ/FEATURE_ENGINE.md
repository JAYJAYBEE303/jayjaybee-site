...
```
RANK_TOP_COUNT         = 15     # fixed count, not a percentage
RANK_TOP_PERCENTILE    = 0.20   # top 20% of the pool
...
```
Tier names describe their **role** (mirroring the `RANK_*` config constant names), not the current threshold number — a name baked to a specific figure (e.g. `'top30'`, `'top10'`) would silently go stale the next time either constant is retuned, exactly as happened when these were first tuned to 30/10% and then retuned to 15/20%.

The three tiers are checked **most-specific first**: `topCount` before `topPercentile` before `bottomPercentile`, even though a top-count player is always also within the top percentile (which is always within the bottom percentile's complement). `topCount` is the fixed-count, "definitely worth considering" flagship signal — checking it first stops it being silently absorbed into the wider percentile tier. **A player outside all three tiers (`rankTier === null`) keeps their existing `BANDS` colour unchanged** — this system only overrides colour for the standout tiers; the unremarkable middle (roughly the 20th–50th percentile) is not the point of the feature and is left alone.

Verified against a realistic 700-player pool at the current thresholds: `topCount` is exactly 15 players (indices 0–14), `topPercentile` is the next 125 (indices 15–139, completing the top 20%), `bottomPercentile` is exactly the bottom 350 (indices 350–699), and the middle 210 are `null`. A pool smaller than `RANK_TOP_COUNT` (e.g. a heavily filtered view) degrades gracefully — everyone in it is `topCount`, no error.

**Colour tokens (`base.css`):** `bottomPercentile` and `topCount` reuse the existing `--band-brutal` (red) and `--band-great` (green) — no new hex for those two, per CONVENTIONS.md §5.3. `topPercentile` needed a genuinely new colour distinct from both existing greens; the first attempt (`--band-lime`, `#a3e635`, a true lime) read too bright against the app's dark palette, so it was replaced with a softer, more desaturated **`--band-light-green`** (`#6fcf78`) plus `--band-light-green-bg`, added alongside the five existing band tokens, same naming convention. Rendered via three new CSS classes (`components.css`): `.score-chip--rank-green`, `.score-chip--rank-light-green`, `.score-chip--rank-red` — deliberately **not** named after the `BANDS` classes they happen to share a colour with (`.score-chip--great` etc.), because they are a different classification mechanism and conflating the two in the DOM would make a future reader unable to tell "scored 85+" from "ranked top 15" just by reading the class name. A chip renders **both** its `BANDS` class and (when applicable) its rank-tier class together; the rank-tier rules are declared after the five band rules in `components.css` so they win the cascade at equal specificity, with no `!important` and no JS class-stripping needed.

**Player pool scope — the FULL pool, not the currently-filtered view.** Ranker's rank tiers are computed against every loaded player (`store.getPlayers()`), **before** `applyFilters()` narrows the table for display — "top 15 in the game" must mean the same thing whether the position filter is set to "GKP only" or "all", and must match what Dashboard and Planner (which have no filters at all) compute for the same players. Dashboard and Planner each maintain a squad of ~15 players with no natural "pool" of their own, so both compute the same full-pool ranking `rankPlayers(store.getPlayers(), horizon, ctx)` + `attachRankTiers(...)` and cache a `playerId → rankTier` lookup, reused across squad edits.
