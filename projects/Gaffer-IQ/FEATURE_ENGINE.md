...
- **`avgPointsPerGw` (`calcAvgPointsPerGw(player, ctx)`)** — `{ value, estimated, source?, seasonName? }`, average FPL points per gameweek this season. Prefers real per-GW `history[]` from an already-loaded player summary (mean of `history[].points`, `estimated: false`) — FPL's history payload has one entry per elapsed gameweek regardless of whether the player featured (0 points for a blank week), so this is already a true weekly average. Falls back, when no summary is loaded (or its `history[]` is empty), to a **pre-season last-season fallback** (§10.1) while the season genuinely hasn't started, then to `player.totals.points ÷ elapsedGws` once it has, where `elapsedGws` is the count of distinct gameweeks in `ctx.playedFixtures` (`estimated: true`). **Divides by gameweeks elapsed this season, not by games the player actually appeared in** — a player who scored 10 points in GW1 and then missed GW2–4 averages `10/4 = 2.5`, not `10/1 = 10`; dividing by appearances instead would make a fringe player's one big week look like elite sustained output. **Never triggers a bulk player-summary fetch** — uses only `ctx.playerSummariesById`, which is populated lazily as the user clicks into players elsewhere in the app (ARCHITECTURE.md §3 rule 7).
- **`costPerPoint`** — `player.price / avgPointsPerGw.value`, or `null` when `avgPointsPerGw.value` is 0 (never `NaN`/`Infinity`; the ranker displays `null` as "—"). **This is the inverse ratio direction from `valueScore`** (money spent per point vs. points per million) — the two must never be merged or treated as the same figure. `valueScore` answers "how much projected score do I get per pound"; `costPerPoint` answers "how much does each point actually cost". **Reads the exact same `avgPointsPerGw` value the Ranker's Avg Pts/GW column displays** — there is no independent computation, so whenever that column is populated (including via the §10.1 fallback below), Cost/Pt is automatically populated too, with no separate wiring.
...
### 10.1 Pre-season last-season fallback (`calcAvgPointsPerGw`) — TEMPORARY

**Purpose:** during pre-season, `avgPointsPerGw` would otherwise show a flat 0 for every player (and, as a direct consequence, `costPerPoint` would show `null`/"—" for every player too, since it's computed from the same value) — not a bug, but genuinely uninformative, since this season's `totals`/`history[]` really are all zero/empty. This is a **temporary measure**, not a permanent redefinition of the metric: it exists purely to give the Ranker something meaningful to show before real current-season data exists, and disables itself the moment that data starts appearing.

**Trigger condition:** the fallback activates only when **both** are true:
1. No real per-GW `history[]` is loaded for this player (the existing, unchanged first branch).
2. `elapsedGws <= 0` — literally no fixture anywhere has finished this season yet. This reuses the exact signal the ordinary season-totals fallback already computed (`ctx.playedFixtures`, deduplicated by `gw`) rather than inventing a new "is it pre-season" check or reading `events[]` separately. The instant a single fixture anywhere finishes, `elapsedGws` becomes ≥ 1 and this fallback stops firing pool-wide.

When active, it sources the player's most recent **past** season from `historyPast` (via `normalisePlayerSummary` — same lazily-loaded `element-summary` fetch that already provides current-season `history[]`, so **no new fetch, no bulk-fetch**; ARCHITECTURE.md §3 rule 7 / §12 non-goal 7 are unaffected): `lastSeasonPoints ÷ SEASON_GWS` (config.js, 38). Flags `estimated: true` (existing convention) **and** `source: 'lastSeason'` / `seasonName` so the UI can render a tooltip distinguishing "last season's number" from the ordinary "thin current-season data" estimate — same `~` marker either way, different explanation underneath.

**Self-disabling, two ways, no manual toggle:**
- **Per-player** — the moment THIS player's own summary loads with real current-season `history[]` entries, the first branch already takes over, independent of every other player and independent of `elapsedGws`.
- **Global** — the moment any fixture anywhere finishes, `elapsedGws` flips to ≥ 1 and the fallback stops firing for *everyone*, including a player who never plays. This matters: a purely per-player "has this player recorded minutes yet" gate would keep an unused player pinned to a stale last-season number indefinitely, since their minutes would never leave 0. Gating on `elapsedGws` instead means that once the season is genuinely underway, an unused player correctly reverts to a real 0 (true information) rather than a misleading old number.

**Worked example, real data** (fetched live from `bootstrap-static`/`element-summary`, player id 12, "Saka" — 0 of 38 events `finished: true` at fetch time, i.e. genuinely pre-season):

| | Before this fallback | After |
|---|---|---|
| `history[]` (current season) | empty | empty (unchanged) |
| `elapsedGws` | 0 | 0 (unchanged) |
| `historyPast` most recent entry | *(ignored)* | `{ seasonName: '2025/26', points: 157 }` |
| `avgPointsPerGw.value` | **0** (the `elapsedGws <= 0` guard forces this regardless of any other field) | `157 / 38 = 4.13` → displayed **4.1**, `estimated: true`, `source: 'lastSeason'` |
| `costPerPoint` (price = £9.5m, `now_cost: 95`) | `null` → displayed **"—"** | `9.5 / 4.131579 = 2.30` → displayed **£2.30m** |

Confirms directly: fixing the average also auto-populates Cost/Pt, with no separate change needed to `costPerPoint` itself — it was already reading the same value.
